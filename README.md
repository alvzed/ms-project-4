...
- Reviews - A commenting system with the basic CRUD functionality, with the update and delete functions limited to users who left the original review. It stores a rating of 1 to 5 and a description where users can elaborate on their thoughts about the video.
...
- Video ratings - would allow users to submit a rating that would be used for a list of good and bad ratings to create an average score of a video. This would use the ratings from the reviews left on the videos.
- Recommendations - will use a user's click and view history to create recommendations for the user.
- Review ratings - this will store the number of thumbs up and thumbs down on the review, and then sort the reviews based on the number and percentage of said thumbs up and thumbs down.
...
The currently unused thumbup and thumbsdown in the Review model are also there for a future rating system, where they will store an integer and sort according to approval numbers.
